if (isMiniTalkIncluded === undefined) {
	...
	var MinitalkComponent = function(opt) {
		...
		this.getPluginPath = function(name) {
			return GetScriptPath().replace("/script","")+"/plugin/"+name;
		}
		
		this.getRootPath = function() {
			return GetScriptPath().replace("/script","");
		}
		...
		this.initTool = function() {
			var defaultTool = [{
				...
				fn:function(minitalk) {
					...
					$(".chatArea").html("");
					minitalk.printMessage("system",LANG.action.clearLog);
				}
			}];
			...
			for (var i=0, loop=m.toolList.length;i<loop;i++) {
				if (typeof m.toolList[i] == "object") {
					...
					if (m.toolType != "text") {
						var iconPath = m.toolList[i].cls ? m.getRootPath()+'/skin/'+m.skin+'/images/'+m.toolList[i].icon : m.toolList[i].icon;
		...
		$(".toggleUserList").on("click",function() {
			if (m.viewUserListStatus == true) {
				...
				m.send("userlist",m.viewUserLimit);
			}
		});
		...
		this.userTag = function(user,isUserList) {
			...
			if (user.opper == "ADMIN") {
				sHTML+= '<img src="'+m.getRootPath()+'/skin/'+m.skin+'/images/icon_admin.gif" />';
			}
		...
		this.printUserMenu = function(userinfo) {
			...
			for (var i=0, loop=m.userMenuList.length;i<loop;i++) {
				if (typeof m.userMenuList[i].viewMenu !== "function" || m.userMenuList[i].viewMenu(m,user,m.myinfo) == true) {
					...
					thisMenu.css("backgroundImage","url("+m.getRootPath()+"/skin/"+m.skin+"/images/"+m.userMenuList[i].icon+")");
		...
		this.checkServer = function(isForce) {
			...
			$.ajax({
				type:"POST",
				url:m.getRootPath()+"/exec/CheckServer.do.php?rnd="+Math.random(),
				...
				success:function(result) {
					if (result.success == true) {
						if (result.server !== false) {
							...
							m.printMessage("error",LANG.errorcode.code201+"(ErrorCode : 201)");
							m.reconnect();
						}
					} else {
						if (result.errorCode) {
		...
		this.send = function(protocol,object) {
			if (protocol != "connect" && (m.socket == null || m.connected == false)) {
				return;
			}
		...
		this.sendMyInfo = function() {
			m.send("connect",{
				channel:m.private != null ? m.private : m.channel,
		...
		this.setEvent = function() {
			...
			this.socket.on("userlist",function(data) {
				m.printUser(data.list);
				m.printUserCount(data.usercount);
			});
			
			this.socket.on("loglist",function(data) {
				m.setLog("chat",data);
			});
